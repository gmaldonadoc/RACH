...
CqiManager::SetLastSent() {
  m_lastSent = round(Simulator::Init()->Now() * 1000);
}
...
CqiManager::NeedToSendFeedbacks(void) {
  if ((GetSendCqi() && GetReportingInterval() == 1) || (GetLastSent() == 0)) {
    return true;
  } else if (GetSendCqi() && ( round(Simulator::Init()->Now() * 1000) - GetLastSent()) >= GetReportingInterval()) {
    return true;
  } else {
    return false;
  }
