...
void
NetworkNode::ReceivePacketBurst(PacketBurst *p) {
#ifdef TEST_DEVICE_ON_CHANNEL
  ...
  for (it = packets.begin(); it != packets.end(); it++) {
    ...
    if (packet->GetDestinationMAC() == GetIDNetworkNode()) {
      ENodeB *eNB = NULL;
      if (GetPhy()->GetDevice()->GetNodeType() == NetworkNode::TYPE_UE) {
        eNB = (ENodeB *) ((UserEquipment *) GetPhy()->GetDevice())->GetTargetNode();
      } else {
        eNB = (ENodeB*) GetPhy()->GetDevice();
        ENodeB::UserEquipmentRecord *record = eNB->GetUserEquipmentRecord(packet->GetSourceMAC());
      ...
      if (packet->GetRLCHeader() == NULL) {
        ...
        RadioBearerInstance *bearer = GetProtocolStack()->GetRrcEntity()->GetRadioBearer(rlcEntityIndex);

        if (GetPhy()->GetDevice()->GetNodeType() == NetworkNode::TYPE_UE) {
          if (((UserEquipment*) GetPhy()->GetDevice())->GetDeviceType() == 0) {
            std::cerr << Simulator::Init()->Now() << " UE HTC " << GetPhy()->GetDevice()->GetIDNetworkNode() << " AC " << ((UserEquipment*) GetPhy()->GetDevice())->GetDeviceAccessClass() << " QCI " << GetPhy()->GetDevice()->GetProtocolStack()->GetMacEntity()->GetQCINumber() << " received Data " << std::endl;
          } else {
            std::cerr << Simulator::Init()->Now() << " UE MTC " << GetPhy()->GetDevice()->GetIDNetworkNode() << " AC " << ((UserEquipment*) GetPhy()->GetDevice())->GetDeviceAccessClass() << " QCI " << GetPhy()->GetDevice()->GetProtocolStack()->GetMacEntity()->GetQCINumber() << " received Data " << std::endl;
          }
        }
