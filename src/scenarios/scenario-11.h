...
static void Scenario_11(int seed, int raMethod, double radius, int ulBW, int dlBW, int pdcchScheduler, int maxMSGs4ToSchedule, int ulTDScheduler, int nUsersToFDScheduler, int ulFDScheduler, int semiSch, int nUE_HTC_VoIP, int nUE_HTC_Video_GBR, int nUE_HTC_Video_nGBR, int nUE_HTC_CBR, int nUE_MTC_TimeDriven, int nUE_MTC_Emergency) {
  ...
  enb->GetPdcchScheduler()->SetnUEsToDL(nUsersToFDScheduler);
  ...
  int nUEs = nUE_HTC_VoIP + nUE_HTC_Video_GBR + nUE_HTC_Video_nGBR + nUE_HTC_CBR + nUE_MTC_TimeDriven + nUE_MTC_Emergency;
  int posX_ue;
  int posY_ue;
  ...
  for (i = 0; i < nUE_HTC_Video_GBR; i++) {
    ...
    ue->SchedulingRequestConfiguration(true, 1);
  ...
  if (nUE_MTC_TimeDriven > 0) {
    ...
    for (itSlot = 0; itSlot < MTCArrivals->size(); itSlot++) {
      ...
      for (i = 0; i < MTCArrivals->at(itSlot)->getNumberArrivals(); i++) {
        ...
        TimeDrivenApp->SetInterval(2.000);
  ...
  simulator->SetStop(stopTime + 5);

  /*
   * Initialize DRX cycle of all UE attached to that eNB.
   * Important: It must go after the declaration of all UE and
  ...
  for (iter = networkManager->GetUserEquipmentContainer()->begin(); iter != networkManager->GetUserEquipmentContainer()->end(); iter++) {
    counter++;
    uepointer = *iter;
    uepointer->GetConsumptionModel()->setConsumo();
    consumoue = uepointer->GetConsumptionModel()->getConsumo();
    ...
    std::cerr << "UE #: " << idue
            << " Energia Consumida: " << consumoue
            << " Tempo Rx: " << timerRx
            << " Tempo RxTx: " << timerRxTx
            << " Tempo Tx: " << timerTx
            << " Tempo NoData: " << timerNoData
            << " Tempo Light: " << timerLightSleep
            << " Tempo Deep: " << timerDeepSleep
            << " Tempo LightActive: " << timerLightActive
            << std::endl;
    std::cerr << "--------------------------" << std::endl;

  }

  std::cerr << "Consumo total: " << consumoTotal << std::endl;
