...
static void Scenario_07(int seed, int raMethod, double radius, int ulBW, int dlBW, int pdcchScheduler, int maxMSGs4ToSchedule, int ulTDScheduler, int nUsersToFDScheduler, int ulFDScheduler, int semiSch, int nUE_HTC_VoIP, int nUE_HTC_Video_GBR, int nUE_HTC_Video_nGBR, int nUE_HTC_CBR, int nUE_HTC_Game, int nUE_MTC_TimeDriven, int nUE_MTC_Emergency) {
  int nUEs = nUE_HTC_VoIP + nUE_HTC_Video_GBR + nUE_HTC_Video_nGBR + nUE_HTC_CBR + nUE_HTC_Game + nUE_MTC_TimeDriven + nUE_MTC_Emergency;
  int posX_ue;
  int posY_ue;
  ...
  enb->GetRrmEntity()->GetPdcchScheduler()->SetnUEsToDL(nUsersToFDScheduler);
  ...
  switch (ulTDScheduler) {
    ...
    case 1: // Proportional Fair
      ...
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetnUsersFD(nUsersToFDScheduler);
      break;
    case 2: // Maximum Throughput
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetUplinkTDScheduler(ENodeB::ULScheduler_TYPE_MAXIMUM_THROUGHPUT);
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetnUsersFD(nUsersToFDScheduler);
      break;
    case 3: // Z-Base QoS
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetUplinkTDScheduler(ENodeB::ULScheduler_TYPE_ZBQoS);
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetnUsersFD(nUsersToFDScheduler);
      break;
  }
  ...
  switch (ulFDScheduler) {
    ...
    case 1: // FME Proportional Fair
      ...
      break;
    case 2: // FME Maximum Throughput
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetUplinkFDScheduler(ENodeB::ULScheduler_TYPE_MAXIMUM_THROUGHPUT);
      break;
    case 3:
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetUplinkFDScheduler(ENodeB::ULScheduler_TYPE_ROUNDROBIN);
      break;
    case 4:
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetUplinkFDScheduler(ENodeB::ULScheduler_TYPE_LIOUMPAS_A1);
      break;
    case 5:
      ((UplinkPacketScheduler *) enb->GetRrmEntity()->GetUplinkPacketScheduler())->SetUplinkFDScheduler(ENodeB::ULScheduler_TYPE_LIOUMPAS_A2);
      break;
  }
  ...
  for (i = 0; i < nUE_HTC_VoIP; i++) {
    ...
    /* used to active SR in VoIP -- true is to SR active */
    ue->SchedulingRequestConfiguration(true, 5);
  ...
  for (i = 0; i < nUE_HTC_Video_GBR; i++) {
    ...
    /* used to active SR in Video -- true is to SR active */
    ue->SchedulingRequestConfiguration(true, 5);
  ...
  for (i = 0; i < nUE_HTC_CBR; i++) {
    ...
    UserEquipment *ue = new UserEquipment(idUe, posX_ue, posY_ue, speed, direction, cell, enb, false, Mobility::RANDOM_DIRECTION);
  ...
  for (i = 0; i < nUE_HTC_Game; i++) {
    ...
    GameUplink *GameApp = new GameUplink();
    GameApp->SetApplicationID(applicationID);
    GameApp->SetSource(ue);
    GameApp->SetDestination(enb);
  ...
  if (nUE_MTC_TimeDriven > 0) {
    ...
    for (itSlot = 0; itSlot < MTCArrivals->size(); itSlot++) {
      ...
      for (i = 0; i < MTCArrivals->at(itSlot)->getNumberArrivals(); i++) {
        ...
        //std::cerr << "Distance node " << idUe << " = " << ue->GetMobilityModel()->GetAbsolutePosition()->GetDistance(enb->GetMobilityModel()->GetAbsolutePosition()) << " " << startTime << std::endl;
  ...
  for (i = 0; i < nUE_MTC_Emergency; i++) {
    ...
    TimeDrivenApp->SetDestinationPort(dstPort);
    TimeDrivenApp->SetTransportProtocol(TransportProtocol::TRANSPORT_PROTOCOL_TYPE_UDP);
    TimeDrivenApp->SetStartTime(stopTime / 4);
    TimeDrivenApp->SetStopTime(stopTime);
    TimeDrivenApp->SetInterval(stopTime / 2);
    TimeDrivenApp->SetSize(10);
