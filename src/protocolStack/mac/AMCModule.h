...
public:
  ...
  int GetImcsFromSinr(double sinr);
  double GetSinrFromImcs(int _mcs);
  ...
  int GetTBSizeToUplinkFromMCS(int mcs, int nbRBs);

  double GetEfficiencyFromCQI(int cqi);
  int GetCQIFromSinr(double sinr);
  double GetSinrFromCQI(int cqi);

  int GetAggregationLevelFromCqi(int cqi);
  int GetPdcchAggregationLevelFromSinr(double sinr);
  int GetPdcchAggregationLevelFromRsrp(double rsrp);
