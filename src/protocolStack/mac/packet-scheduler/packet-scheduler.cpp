...
double
PacketScheduler::FlowToSchedule::GetSpectralEfficiency(int index) {
  return m_spectralEfficiency[index];
}
...
void
PacketScheduler::FlowToSchedule::SetAllowToSchedule(bool allow) {
  m_allowToSchedule = allow;
}

bool
PacketScheduler::FlowToSchedule::GetAllowToSchedule() {
  return m_allowToSchedule;
}

void
PacketScheduler::FlowToSchedule::SetMaximumDelay(double delay) {
  m_maximumDelay = delay;
}

double
PacketScheduler::FlowToSchedule::GetMaximumDelay() {
...
void
PacketScheduler::InsertFlowToSchedule(RadioBearer *bearer, int dataToTransmit, std::vector<double> specEff, std::vector<int> cqiFeedbacks, bool allowToSchedule) {
#ifdef SCHEDULER_DEBUG
  std::cout << "\t  --> selected flow: "
          << bearer->GetApplication()->GetApplicationID()
          << " " << dataToTransmit << std::endl;
  ...
  flowToSchedule->m_averageUERate = bearer->GetAverageTransmissionRate() / 1000;
  flowToSchedule->m_gbr = bearer->GetQoSParameters()->GetGBR();
  flowToSchedule->m_realHoL = bearer->GetHeadOfLinePacketDelay();
  flowToSchedule->m_maximumDelay = bearer->GetQoSParameters()->GetMaxDelay();
  flowToSchedule->m_allowToSchedule = allowToSchedule;
