    ...
    if (selectedUser != -1) {
      ...
      while ((scheduledUser->m_dataToTransmit * 8 > (amc->GetTBSizeToUplinkFromMCS(mcs, prbs))) && (availableRBs > 0)) {
        ...
        if (((right < nbOfRBs) && (!Allocated[right]) && (bestUserForRB[right] == selectedUser))
                && (((left >= 0) && (metricsFD[right][selectedUser] >= metricsFD[left][selectedUser]))
                || (left < 0)
                || Allocated[left])) {
          ...
          last = right - 1;
                ...
                && (((right < nbOfRBs) && (metricsFD[left][selectedUser] > metricsFD[right][selectedUser]))
                || (right >= nbOfRBs)
                || Allocated[right])) {
...
#ifdef SCHEDULER_DEBUG
      std::cout << "[DEBUG] UE " << selectedUser << " " << scheduledUser->m_userToSchedule->GetIDNetworkNode()
