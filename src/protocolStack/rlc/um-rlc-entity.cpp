...
#ifdef RLC_DEBUG
  std::cout << "UM RLC tx procedure for node " << GetRadioBearerInstance()->GetSource()->GetIDNetworkNode() << " at " << Simulator::Init()->Now() << std::endl;
#endif
...
UmRlcEntity::ReceptionProcedure(Packet *p) {
#ifdef RLC_DEBUG
  std::cout << "UM RLC rx procedure for node " << GetRadioBearerInstance()->GetDestination()->GetIDNetworkNode() << " at " << Simulator::Init()->Now() << std::endl;
  std::cout << "RECEIVE PACKET id " << p->GetID() << " frag n " << p->GetRLCHeader()->GetFragmentNumber() << std::endl;
#endif

  if (_RLC_TRACING_) {
    std::cout << "UM_RLC SIZE " << p->GetSize() <<
            " B " << GetRlcEntityIndex() <<
  ...
  if (m_incomingPacket.size() > 0 && p->GetID() != m_incomingPacket.at(0)->GetID()) {
    ...
    if (_RLC_TRACING_) {
      ...
      if (pp->GetPacketTags()->GetApplicationType() == PacketTAGs::APPLICATION_TYPE_VOIP)
        ...
        std::cout << " INF_BUF";
      else if (p->GetPacketTags()->GetApplicationType() == PacketTAGs::APPLICATION_TYPE_WEB)
        std::cout << " WEB";
      ...
      else if (p->GetPacketTags()->GetApplicationType() == PacketTAGs::APPLICATION_TYPE_GAME)
        std::cout << " GAME";
      else
        std::cout << " UNKNOW";
  ...
  if (!p->GetRLCHeader()->IsAFragment()) {
    ...
    RadioBearerSink *bearerSink = (RadioBearerSink*) GetRadioBearerInstance();
    bearerSink->Receive(p);
  }
