...
AmRlcEntity::PrintSentAMDs(void) {
  ...
  for (it = m_sentAMDs->begin(); it != m_sentAMDs->end(); it++) {
    ...
    std::cout << "\t\t *** pkt " << packet->GetID() << " frag " << packet->GetRLCHeader()->GetFragmentNumber() << " sn " << packet->GetRLCHeader()->GetRlcPduSequenceNumber() << " startB " << packet->GetRLCHeader()->GetStartByte() <<
...
AmRlcEntity::TransmissionProcedure(int availableBytes) {
#ifdef RLC_DEBUG
  std::cout << "AM RLC tx procedure for node " << GetRadioBearerInstance()->GetSource()->GetIDNetworkNode() << " bearer " << GetRlcEntityIndex() << " at " << Simulator::Init()->Now() << std::endl;
#endif
  ...
  if (nbSentAMDs != 0) {
    ...
    while (availableBytes > 0 && amdId < nbSentAMDs) {
      ...
      if (amdRecord->m_packet->GetSize() + 6 <= availableBytes) //6 = MAC  + CRC overhead
      {
        ...
        MACHeader *mac = new MACHeader(GetRadioBearerInstance()->GetSource()->GetIDNetworkNode(), GetRadioBearerInstance()->GetDestination()->GetIDNetworkNode());
        p->AddMACHeader(mac);
        p->AddHeaderSize(3);
  ...
  if (bearer->GetApplication()->GetApplicationType() == Application::APPLICATION_TYPE_INFINITE_BUFFER) {
    ...
    while (availableBytes > 0 && !queue->IsEmpty()) {
      std::cout << "RLC Available " << availableBytes << std::endl;
...
#ifdef RLC_DEBUG
        std::cout << "_____ pkt " << packet->GetID() << " frag " << packet->GetRLCHeader()->GetFragmentNumber() << " sn " << packet->GetRLCHeader()->GetRlcPduSequenceNumber() << " startB " << packet->GetRLCHeader()->GetStartByte() << " endB " << packet->GetRLCHeader()->GetEndByte() << std::endl;
        PrintSentAMDs();
#endif
...
AmRlcEntity::ReceptionProcedure(Packet* p) {
  ...
  std::cout << "_____ pkt " << p->GetID() << " frag " << p->GetRLCHeader()->GetFragmentNumber() <<
          ...
          " endB " << p->GetRLCHeader()->GetEndByte() << " at " << Simulator::Init()->Now() << std::endl;
#endif
...
AmRlcEntity::ReceptionProcedureEnd() {
  ...
  for (it = m_receivedAMDs->begin(); it != m_receivedAMDs->end(); it++) {
    ...
    if (packet->GetID() != currentPacket && packet->GetRLCHeader()->GetStartByte() == 0) {
      //std::cerr << "\t\t !! first fragment of the packet" << std::endl;
      ...
      //std::cerr << "\t\t !! fragment of a new packet, but not he first" << std::endl;
      expectedNextByte = 0;
      ...
      if (packet->GetRLCHeader()->IsTheLatestFragment()) {
        //std::cerr << "\t\t !! consecutive fragment --> the packet can be reassembled" << std::endl;
        RadioBearerSink *bearer = (RadioBearerSink*) GetRadioBearerInstance();
        ...
        //std::cerr << "\t\t !! consecutive fragment --> we expect another fragment" << std::endl;
        expectedNextByte = packet->GetRLCHeader()->GetEndByte() + 1;
      }
    }
  }
...
AmRlcEntity::ReceiveArqRlcIdealControlMessage(ArqRlcIdealControlMessage msg) {
  ...
  for (it = m_sentAMDs->begin(); it != m_sentAMDs->end(); it++) {
    ...
    if (amdRecord->m_packet->GetRLCHeader()->GetRlcPduSequenceNumber() == msg.GetAck() && amdRecord->m_packet->GetRLCHeader()->GetStartByte() == msg.GetStartByte()) {
      m_sentAMDs->erase(it);
      delete amdRecord;
