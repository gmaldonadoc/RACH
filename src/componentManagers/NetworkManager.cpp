...
NetworkManager::NetworkManager()
{
  ...
  m_userEquipmentsDoingRandomAccess = new UserEquipmentsDoingRandomAccess;
}

NetworkManager::~NetworkManager()
{
  std::vector<Cell *>::iterator iter;
  for (iter = m_cellContainer->begin(); iter != m_cellContainer->end(); iter++)
  {
    delete *iter;
  }
  ...
  for (iter4 = m_gatewayContainer->begin(); iter4 != m_gatewayContainer->end(); iter4++)
  {
    delete *iter4;
  }
  ...
  for (iter2 = m_eNodeBContainer->begin(); iter2 != m_eNodeBContainer->end(); iter2++)
  {
    delete *iter2;
  }
  ...
  for (iter3 = m_userEquipmentContainer->begin(); iter3 != m_userEquipmentContainer->end(); iter3++)
  {
    delete *iter3;
  }
...
ENodeB *
NetworkManager::CreateEnodeb(int id, Cell *cell, double pos_X, double pos_Y, LteChannel *dlCh, LteChannel *ulCh, BandwidthManager *bm)
{
  ENodeB *enb = new ENodeB(id, cell, pos_X, pos_Y);
...
UserEquipment *
NetworkManager::CreateUserEquipment(int id, double pos_X, double pos_Y, double speed, double speedDirection, Cell *cell, ENodeB *enb)
{
  UserEquipment *ue = new UserEquipment(id, pos_X, pos_Y, speed, speedDirection, cell, enb, 0, Mobility::RANDOM_DIRECTION);
...
void NetworkManager::CreateStreet(int idStreet, CartesianCoordinates *center, double theta,
...
void NetworkManager::CreateBuildingForFemtocells(int idBuilding,
                                                 int buildingType,
...
void NetworkManager::DeleteUserEquipment(UserEquipment *UE)
{
  std::vector<UserEquipment *> *records = GetUserEquipmentContainer();
  std::vector<UserEquipment *>::iterator iter;

  std::vector<UserEquipment *> *new_records = new std::vector<UserEquipment *>;

  for (iter = records->begin(); iter != records->end(); iter++)
  {
    if (((UserEquipment *)*iter)->GetIDNetworkNode() != UE->GetIDNetworkNode())
    {
      new_records->push_back(*iter);
    }
    else
    {
      delete *iter;
  ...
  m_userEquipmentContainer->clear();
  delete m_userEquipmentContainer;
  m_userEquipmentContainer = new_records;
}
...
NetworkManager::GetCellIDFromPosition(CartesianCoordinates *position)
{
  ...
  {
    cell = *iter;
    distance = sqrt(pow((cell->GetCellCenterPosition()->GetCoordinateX() - position->GetCoordinateX()), 2) + pow((cell->GetCellCenterPosition()->GetCoordinateY() - position->GetCoordinateY()), 2));
...
int NetworkManager::GetCellIDFromPosition(double posX,
                                          double posY)
{
  std::vector<Cell *> *cellContainer = GetCellContainer();
  std::vector<Cell *>::iterator iter;
...
void NetworkManager::UpdateUserPosition(double time)
{
  ...
  for (iter = records->begin(); iter != records->end(); iter++)
  {
...
#ifdef AMC_MAPPING
    std::cout << "time: " << time << "\n\t position: "
              << record->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateX() << " " << record->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateY()
              << std::endl;
#endif
#ifdef MOBILITY_DEBUG
    ...
    if (record->GetMobilityModel()->GetHandover() == true)
    {
      ...
      if (targetNode->GetProtocolStack()->GetRrcEntity()->GetHandoverEntity()->CheckHandoverNeed(record))
      {
        ...
        std::cout << "** HO ** \t time: " << time << " user " << record->GetIDNetworkNode() << " old eNB " << targetNode->GetIDNetworkNode() << " new eNB " << newTagertNode->GetIDNetworkNode() << std::endl;
#endif
...
bool NetworkManager::CheckHandoverPermissions(NetworkNode *target, UserEquipment *user)
{
...
bool NetworkManager::CheckIndoorUsers(UserEquipment *ue)
{
  ...
  if (buildingContainer->size() > 0)
  {
         ...
         iter != buildingContainer->end(); iter++)
    {
      ...
      if ((abs(building->GetCenterPosition()->GetCoordinateX() - ue->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateX()) < building->GetSide()[0] / 2) &&
          (abs(building->GetCenterPosition()->GetCoordinateY() - ue->GetMobilityModel()->GetAbsolutePosition()->GetCoordinateY()) < building->GetSide()[1] / 2))
      {
        return true;
      }
    }
...
void NetworkManager::SelectTargetNode(UserEquipment *ue)
{
  ...
  if (targetNode->GetProtocolStack()->GetRrcEntity()->GetHandoverEntity()->CheckHandoverNeed(ue))
  {
...
void NetworkManager::HandoverProcedure(double time, UserEquipment *ue, NetworkNode *oldTarget, NetworkNode *newTarget)
{
#ifdef HANDOVER_DEBUG
...
void NetworkManager::TransferBearerInfo(UserEquipment *ue, NetworkNode *target)
{
#ifdef HANDOVER_DEBUG
...
NetworkManager::UserEquipmentsDoingRandomAccess *
NetworkManager::GetUserEquipmentsDoingRandomAccess()
{
  return m_userEquipmentsDoingRandomAccess;
}

void NetworkManager::AddUserEquipmentDoingRandomAccess(UserEquipment *UE)
{
  GetUserEquipmentsDoingRandomAccess()->push_back(UE);
}

void NetworkManager::RemoveUserEquipmentDoingRandomAccess(UserEquipment *UE)
{
  UserEquipmentsDoingRandomAccess *records = GetUserEquipmentsDoingRandomAccess();
  UserEquipment *record;
  UserEquipmentsDoingRandomAccess::iterator iter;

  UserEquipmentsDoingRandomAccess *new_records = new UserEquipmentsDoingRandomAccess;

  for (iter = records->begin(); iter != records->end(); iter++)
  {
    record = *iter;
    if (record->GetIDNetworkNode() != UE->GetIDNetworkNode())
    {
      new_records->push_back(record);
    }
  }

  m_userEquipmentsDoingRandomAccess->clear();
  delete m_userEquipmentsDoingRandomAccess;
  m_userEquipmentsDoingRandomAccess = new_records;

  // std::vector<Gateway *> *NetworkManager::GetMGatewayContainer(void)
  // {
  //   return m_gatewayContainer;
  // }
}
