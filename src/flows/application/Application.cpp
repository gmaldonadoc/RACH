...
Application::Start() {
  ...
  std::cout << "Start Application: src: " << GetSource()->GetIDNetworkNode() << " dst: " << GetDestination()->GetIDNetworkNode() << std::endl;
#endif
...
#ifdef TEST_START_APPLICATION
    std::cout << "Create UL radio bearer between: " << GetSource()->GetIDNetworkNode() << " and " << ue->GetTargetNode()->GetIDNetworkNode() << std::endl;
#endif

    m_radioBearer->SetSource(ue);

    if (ue->GetD2DAble()) {
      m_radioBearer->SetDestination(GetDestination());
    } else {
...
#ifdef TEST_START_APPLICATION
    std::cout << "Create DL radio bearer between: " << ue->GetTargetNode()->GetIDNetworkNode() << " and " << ue->GetIDNetworkNode() << std::endl;
#endif
  ...
  if (GetSource()->GetNodeType() == NetworkNode::TYPE_UE) {
    ...
    if (ue->GetD2DAble()) {
      m_bearerSink->SetDestination(this->GetDestination());
    } else {
  ...
  if (GetSource()->GetNodeType() == NetworkNode::TYPE_UE && GetDestination()->GetNodeType() == NetworkNode::TYPE_UE && !((UserEquipment*) GetSource())->GetD2DAble() && !((UserEquipment*) GetDestination())->GetD2DAble()) {
    RadioBearer *radioBearer = new RadioBearer();
    radioBearer->GetRlcEntity()->SetRlcEntityIndex(GetApplicationID());
    radioBearer->SetSource(((UserEquipment*) GetDestination())->GetTargetNode());
    radioBearer->SetDestination(GetDestination());
    radioBearer->SetApplication(this);
    radioBearer->SetClassifierParameters(GetClassifierParameters());
    radioBearer->SetQoSParameters(GetQoSParameters());
    radioBearer->GetSource()->GetProtocolStack()->GetRrcEntity()->AddRadioBearer(radioBearer);

    RadioBearerSink *bearerSink = new RadioBearerSink();
    bearerSink->GetRlcEntity()->SetRlcEntityIndex(GetApplicationID());
    bearerSink->SetSource(((UserEquipment*) GetDestination())->GetTargetNode());
    bearerSink->SetDestination(GetDestination());
    bearerSink->SetApplication(m_applicationSink);
    bearerSink->SetClassifierParameters(GetClassifierParameters());
    bearerSink->SetQoSParameters(GetQoSParameters());

    radioBearer->GetDestination()->GetProtocolStack()->GetRrcEntity()->AddRadioBearerSink(bearerSink);

    m_bearerSink->SetRadioBearer(radioBearer);

    m_applicationSink->SetRadioBearerSink(bearerSink);

    std::cout << "Teste Bi!" << std::endl;
  } else {
    m_applicationSink->SetRadioBearerSink(m_bearerSink);
  }

  // 5 - attach UE on the UL or DL channel
  if (GetSource()->GetNodeType() == NetworkNode::TYPE_UE) {
    UserEquipment* ue = (UserEquipment*) GetSource();
    ue->MakeActive();
...
Application::Trace(Packet* p) {
  ...
  std::cerr << Simulator::Init()->Now() << " TX ";
  ...
  if (GetDestination()->GetNodeType() == NetworkNode::TYPE_UE) {
    UserEquipment* ue = (UserEquipment*) GetDestination();
    std::cerr << " ID " << p->GetID()
            << " BEARE " << GetApplicationID()
            << " QCI " << GetQoSParameters()->GetQCI()
            << " SIZE " << p->GetSize()
