...
void
GameDownlink::DoStart(void) {
  Simulator::Init()->Schedule(0.0, &GameDownlink::Send, this);
}

void
GameDownlink::DoStop(void) {
}

void
GameDownlink::ScheduleTransmit(double time) {
  if ((Simulator::Init()->Now() + time) <= GetStopTime()) {
    Simulator::Init()->Schedule(time, &GameDownlink::Send, this);
  }
}

void
GameDownlink::Send(void) {
  double u = Random::Init()->Uniform(0., 1.);
  int s = 330. - (82. * log(-1 * log(1 - u)));
  SetSize(s);
...
int
GameDownlink::GetSize(void) const {
  return m_size;
}

void
GameDownlink::SetSize(int size) {
  m_size = size;
}

void
GameDownlink::SetInterval(double interval) {
  m_interval = interval;
}

double
GameDownlink::GetInterval(void) const {
  return m_interval;
}