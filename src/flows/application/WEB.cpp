...
double
Random::Exponential(double lamda) {
  double f;
  f = (double) rand() / RAND_MAX;
...
double
Random::Pareto(double a, double k) {
  double f;
  f = (double) rand() / RAND_MAX;
  f = pow(f, 1 / a);
  f = k / f;
...
double
Random::Normal(int n, double Mu, double Theta) {
  double u, t;
  int i;
...
Random::Poisson(double lamda) {
  ...
  do {
    c++;
    n1 = n1 * ((double) rand() / (double) RAND_MAX);
  } while (n1 > R);
  return c;
}

WEB::WEB() {
  m_size = Random::Init()->Pareto(1.1, 81.5);
  if (m_size > 66666) {
    m_size = 66666;
  }
  m_stateON = false;
...
WEB::Send(void) {
  if (!m_stateON) {
    ...
    int n_packet = Random::Init()->Geometric(0.1);
    std::cerr << n_packet << std::endl;
    arrival = Random::Init()->Geometric(0.4);
    ...
    std::cerr << " WEB_DEBUG - Start ON Period, \n\t Time = " << Simulator::Init()->Now() << "\n\t state ON Duration = " << m_stateDuration << "\n\t end ON state = " << m_endState << std::endl;
  }
