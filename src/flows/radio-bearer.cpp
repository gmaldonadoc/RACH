...
RadioBearer::UpdateAverageTransmissionRate() {
  ...
  if ((Simulator::Init()->Now() - GetLastUpdate()) > 0) {
    rate = (GetTransmittedBytes() * 8.) / (Simulator::Init()->Now() - GetLastUpdate());
  } else {
    rate = 0;
  }
...
//#endif
  ...
  int b = GetTransmittedBytes();
  double rate;

  if ((Simulator::Init()->Now() - GetLastUpdate()) != 0) {
    rate = (b * 8) / (Simulator::Init()->Now() - GetLastUpdate());
  } else {
    rate = 0;
  }
  ...
  double averageUERateOld = (double) m_averageTransmissionRate;

  m_averageTransmissionRate = (double) (((1 - beta) * averageUERateOld) + (beta * rate));

  if (m_averageTransmissionRate < 1) {
    m_averageTransmissionRate = 1; //bps
  }
...
RadioBearer::CheckForDropPackets() {
  if (m_application->GetApplicationType() != Application::APPLICATION_TYPE_TRACE_BASED && m_application->GetApplicationType() != Application::APPLICATION_TYPE_VOIP) {
    return;
  }
  ...
  if ((ONLY_RANDOM_ACCESS == false) && (UPLINK == true)) {
    if (GetMacQueue()->IsEmpty()) {
      if (GetSource()->GetNodeType() == NetworkNode::TYPE_UE) {
        ((UeMacEntity*) GetSource()->GetProtocolStack()->GetMacEntity())->TriggerBufferStatusReport(1);
      }
    }
  }
