...
QoSParameters::QoSParameters(int qci) {
  ...
  m_arpPreEmptionCapability = false;
  m_arpPreEmptionVulnerability = false;
  m_maxDelay = packetDelayBudgetOfQCIs[qci - 1];
}
...
QoSParameters::QoSParameters(int qci, bool arpPreEmptionCapability, bool arpPreEmptionVulnerability, double gbr, double mbr) {
  m_qci = qci;
  m_gbr = gbr;
  m_mbr = mbr;
  m_initialRate = 1;
...
/*********** TODO: No Implemented (Carlos A.) ***************/
void
QoSParameters::SetInitialRate(double initialRate) {
  m_initialRate = initialRate; //In kbps
  //std::cout << "Initial Rate Set to: " << m_initialRate << " Kbps" << std::endl;
}

double
QoSParameters::GetInitialRate(void) const {
  return m_initialRate; //In kbps
}
