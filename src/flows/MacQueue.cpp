...
MacQueue::Enqueue(Packet *packet) {
  ...
  //std::cerr << "Size " << element.GetSize() << " - " << GetQueueSize() << std::endl;
...
MacQueue::GetPacketToTramsit(int availableBytes) {
  ...
  std::cout << " MAC_DEBUG: Dequeue(), availableBytes = " << availableBytes << std::endl;
#endif
...
MacQueue::CheckForDropPackets(double maxDelay, int bearerID) {
  ...
  while (true && GetPacketQueue()->size() > 0) {
    ...
    if (headOfLineDelay > maxDelay) {
      //if (_MAC_TRACING_) {
      if (_APP_TRACING_) {
        std::cerr << Simulator::Init()->Now() << " DROP ";
        ...
        std::cerr << " ID " << GetPacketQueue()->begin()->GetPacket()->GetID()
                << " BEARE " << bearerID
                << " SIZE " << GetPacketQueue()->begin()->GetPacket()->GetSize()
                << " SRC " << GetPacketQueue()->begin()->GetPacket()->GetSourceID()
                << " DST " << GetPacketQueue()->begin()->GetPacket()->GetDestinationID();
        ...
        if (GetPacketQueue()->begin()->GetPacket()->GetPacketTags() != NULL
          ...
          std::cerr << " FRAME " << GetPacketQueue()->begin()->GetPacket()->GetPacketTags()->GetFrameNuber()
