...
void
Simulator::Stop(void) {
  //std::cout << " SIMULATOR_DEBUG: Stop ()" << std::endl;
  m_stop = true;
}
...
Simulator::SetStop(double time) {
  m_timeToStop = time;
  DoSchedule(time, MakeEvent(&Simulator::Stop, this));
}

//void

Event*
Simulator::DoSchedule(double time, Event *event) {
  double timeStamp = time + Now();

  //std::cerr << "Timestamp = " << timeStamp << std::endl;

  event->SetTimeStamp(round(timeStamp * 10000.0) / 10000.0);

  ++m_unscheduledEvents;

  //std::cerr << "Events = " << m_unscheduledEvents << std::endl;
