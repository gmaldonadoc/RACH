...
LteChannel::StartTx(PacketBurst *p, TransmittedSignal *txSignal, NetworkNode *src) {
  ...
  Simulator::Init()->Schedule(0.000, &LteChannel::StartRx, this, p, txSignal, src);
...
LteChannel::StartRx(PacketBurst *p, TransmittedSignal *txSignal, NetworkNode *src) {
#ifdef TEST_DEVICE_ON_CHANNEL
  std::cout << "LteChannel::StartRx ch " << GetChannelId() << " src " << src->GetIDNetworkNode() << std::endl;
#endif
  ...
  for (std::vector<NetworkNode*>::iterator it = GetDevices()->begin(); it != GetDevices()->end(); it++) {
    ...
    if (src->GetIDNetworkNode() == dst->GetIDNetworkNode()) {
      continue;
    }
    ...
    if (p == NULL) {
      std::cerr << Simulator::Init()->Now() << " NULL" << std::endl;
    } else {
