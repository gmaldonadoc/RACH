...
TransmittedSignal*
PropagationLossModel::AddLossModel(NetworkNode *src, NetworkNode *dst, TransmittedSignal *txSignal) {
  ...
  for (int i = 0; i < nbOfSubChannels; i++) {
    ...
    //double rxPower = rxSignalValues.at(i);

#ifdef TEST_PROPAGATION_LOSS_MODEL
    std::cout << "\t\t sub channel = " << i << " rxSignalValue = " << rxSignalValues.at(i) << " loss = " << loss.at(i) << " rxPower = " << rxPower << std::endl;
#endif

    rxSignalValues.at(i) = rxPower;
  }
