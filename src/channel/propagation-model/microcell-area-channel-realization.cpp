...
MicroCellAreaChannelRealization::GetPathLoss(void) {
   ...
   * the Path Loss Model For Urban-microcell Environment is
   * for normal communication => L = 34.5 + 38 * log10(d) -- d (in meters) is the distance between two nodes
   * for D2D communications => L = 37 + 30 * log10(d) -- d (in meters) is the distance between two nodes
   */
  ...
  /*if (GetSourceNode()->GetNodeType() == NetworkNode::TYPE_UE && GetDestinationNode()->GetNodeType() == NetworkNode::TYPE_ENODEB) {
  ...
  } else if (GetDestinationNode()->GetNodeType() == NetworkNode::TYPE_UE && GetSourceNode()->GetNodeType() == NetworkNode::TYPE_ENODEB) {
  ...
  if (((src->GetNodeType() == NetworkNode::TYPE_UE) && (dst->GetNodeType() == NetworkNode::TYPE_ENODEB)) || ((src->GetNodeType() == NetworkNode::TYPE_ENODEB) && (dst->GetNodeType() == NetworkNode::TYPE_UE))) {
    m_pathLoss = 34.5 + (38 * log10(distance));
  } else {
