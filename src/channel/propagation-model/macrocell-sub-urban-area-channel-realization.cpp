...
MacroCellSubUrbanAreaChannelRealization::GetPathLoss(void) {
   ...
   * //L = 128.1 + 37.6 * log10(d) -- d (in kilometers) is the distance between two nodes
   * L = 31.5 + 35 * log10(d) -- d (in kilometers) is the distance between two nodes -- C = 0dB, henb = 32m, hue = 1.5m and fc = 1.9GHz
   */
  double distance;
  ...
  if (GetSourceNode()->GetNodeType() == NetworkNode::TYPE_UE && GetDestinationNode()->GetNodeType() == NetworkNode::TYPE_ENODEB) {
    ...
    m_pathLoss = 30.5 + (35.2 * log10(distance * 0.001));
  }
  else if (GetSourceNode()->GetNodeType() == NetworkNode::TYPE_ENODEB && GetDestinationNode()->GetNodeType() == NetworkNode::TYPE_UE) {
    UserEquipment* ue = (UserEquipment*) GetDestinationNode();
    ENodeB* enb = (ENodeB*) GetSourceNode();

    distance = ue->GetMobilityModel()->GetAbsolutePosition()->GetDistance(enb->GetMobilityModel()->GetAbsolutePosition());
    
    m_pathLoss = 32.0 + (35.2 * log10(distance * 0.001));
  }
