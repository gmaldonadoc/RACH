...
Interference::ComputeInterference(UserEquipment *ue) {
  ...
  for (it = eNBs->begin(); it != eNBs->end(); it++) {
    node = (*it);
    if (node->GetIDNetworkNode() != ue->GetTargetNode()->GetIDNetworkNode() && node->GetPhy()->GetBandwidthManager()->GetDlOffsetBw() == ue->GetTargetNode()->GetPhy()->GetBandwidthManager()->GetDlOffsetBw()) {
      double powerTXForSubBandwidth = 10 * log10(pow(10., (node->GetPhy()->GetTxPower() - 30) / 10) / node->GetPhy()->GetBandwidthManager()->GetDlSubChannels().size());
  ...
  for (it2 = HeNBs->begin(); it2 != HeNBs->end(); it2++) {
    henb = (*it2);
    if (henb->GetIDNetworkNode() != ue->GetTargetNode()->GetIDNetworkNode() && henb->GetPhy()->GetBandwidthManager()->GetDlOffsetBw() == ue->GetTargetNode()->GetPhy()->GetBandwidthManager()->GetDlOffsetBw()) {
      double powerTXForSubBandwidth = 10 * log10(pow(10., (henb->GetPhy()->GetTxPower() - 30) / 10) / henb->GetPhy()->GetBandwidthManager()->GetDlSubChannels().size());
