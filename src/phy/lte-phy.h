...
public:
  ...
  virtual void EndRx(PacketBurst *p, TransmittedSignal *txSignal) = 0;

  void SetDevice(NetworkNode* d);
  NetworkNode* GetDevice(void);

  void SetDlChannel(LteChannel* ch);
  ...
  void SetMaxTxPower(double power);
  ...
  virtual void ReceiveIdealControlMessage(IdealControlMessage *msg, NetworkNode *src, TransmittedSignal *txSignal) = 0;
  ...
  double CreateNoisePower(int nSubcarriers);

  virtual void ReceiveSoundingReferenceSignal(NetworkNode *n, TransmittedSignal *s) = 0;
